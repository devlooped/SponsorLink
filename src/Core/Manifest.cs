...
static partial class Manifest
{
    ...
    public static bool TryRead([NotNullWhen(true)] out ClaimsPrincipal? principal, IEnumerable<(string? jwt, string? publicKey)> values)
    {
        principal = null;

        foreach (var value in values)
        {
            if (string.IsNullOrWhiteSpace(value.jwt) || string.IsNullOrWhiteSpace(value.publicKey))
                continue;
